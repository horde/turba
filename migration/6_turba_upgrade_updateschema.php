...
class TurbaUpgradeUpdateSchema extends Horde_Db_Migration_Base
{
    ...
    public function up()
    {
        ...
        if (!in_array('object_anniversary', $cols)) {
            $this->addColumn('turba_objects', 'object_anniversary', 'string', ['limit' => 10]);
        }
        if (!in_array('object_department', $cols)) {
            $this->addColumn('turba_objects', 'object_department', 'string', ['limit' => 255]);
        }
        if (!in_array('object_spouse', $cols)) {
            $this->addColumn('turba_objects', 'object_spouse', 'string', ['limit' => 255]);
        }
        if (!in_array('object_homefax', $cols)) {
            $this->addColumn('turba_objects', 'object_homefax', 'string', ['limit' => 25]);
        }
        if (!in_array('object_nickname', $cols)) {
            $this->addColumn('turba_objects', 'object_nickname', 'string', ['limit' => 255]);
        }
        if (!in_array('object_assistantphone', $cols)) {
            $this->addColumn('turba_objects', 'object_assistantphone', 'string', ['limit' => 25]);
        }
        if (!in_array('object_imaddress', $cols)) {
            $this->addColumn('turba_objects', 'object_imaddress', 'string', ['limit' => 255]);
        }
        if (!in_array('object_imaddress2', $cols)) {
            $this->addColumn('turba_objects', 'object_imaddress2', 'string', ['limit' => 255]);
        }
        if (!in_array('object_imaddress3', $cols)) {
            $this->addColumn('turba_objects', 'object_imaddress3', 'string', ['limit' => 255]);
        }
    }
